...
// This can be called from a different thread than the Present() thread,
// so we need to juggle the should_show_ui flag to prevent a race condition
// where dx11 gets initialized again before resizebuffers completes
static HRESULT FAKE_ResizeBuffers(IDXGISwapChain3* This, UINT BufferCount, UINT Width, UINT Height, DXGI_FORMAT NewFormat, UINT SwapChainFlags) {
        const bool previously_shown = should_show_ui;
        if (should_show_ui) {
        ...
        should_show_ui = previously_shown;
        return ret;
}


static HRESULT FAKE_Present(IDXGISwapChain3* This, UINT SyncInterval, UINT PresentFlags) {
        if (DebugEveryNFrames(1200)) {
